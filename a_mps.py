...
class SimpleMPS:
    ...
    def site0_expectation_value(self, op):
        """Calculate expectation values of a local operator at first site."""
        result = []
        
        theta = self.get_theta1(0)  # vL i vR
        op_theta = np.tensordot(op, theta, axes=[1, 1])  # i [i*], vL [i] vR
        result.append(np.tensordot(theta.conj(), op_theta, [[0, 1, 2], [1, 0, 2]]))
        # [vL*] [i*] [vR*], [i] [vL] [vR]
        return np.real_if_close(result)
    ...
    def two_site_expectation_value(self, op2):
        """Expectation value of the same 2-site operator on every bond.

        op2 has legs (i, j, i*, j*), i.e., shape (d, d, d, d).
        Returns a list of length self.nbonds with ⟨op2⟩ on each bond.
        """
        result = []
        for i in range(self.nbonds):
            theta = self.get_theta2(i)                              # vL i j vR
            op_theta = np.tensordot(op2, theta, axes=[[2, 3], [1, 2]])  # i j [i*] [j*], vL [i] [j] vR
            val = np.tensordot(theta.conj(), op_theta, [[0, 1, 2, 3], [2, 0, 1, 3]])
            result.append(val)
        return np.real_if_close(result)


    def two_site_rdm(self, i, normalize=True):
        """Compute the two-site reduced density matrix ρ_{i,i+1}.

        Parameters
        ----------
        i : int
            Index of the left site in the pair (i, i+1).
        normalize : bool
            Whether to normalize so that Tr(ρ)=1.

        Returns
        -------
        rho : np.ndarray
            The two-site reduced density matrix of shape (d**2, d**2).
        """
        j = (i + 1) % self.L
        # Two-site wavefunction in mixed canonical form: vL i j vR
        theta = self.get_theta2(i)
        d = theta.shape[1]

        # Reshape to (phys^2, environment)
        theta = np.transpose(theta, (1, 2, 0, 3))  # (i, j, vL, vR)
        theta = theta.reshape(d * d, -1)

        # Trace out the environment: ρ = Θ Θ†
        rho = theta @ theta.conj().T
    ...
    def correlation_length(self):
        ...
        eta = scipy.sparse.linalg.eigsh(T, k=2, which='LM', return_eigenvectors=False, ncv=20)
...
def init_FM_MPS(L, d=2, bc='finite'):
    ...
    return SimpleMPS(As, Ss, bc=bc)

def init_Gen_MPS(L, d, chi, bc='finite'):
    """Return a generic MPS with random entries"""
    B = np.zeros([1, d, 1], float)
    B[0, 0, 0] = np.random.rand()
    #B[0, 3, 0] = 1
    #i0 = int(0.5 * d + 1)
    #i0 = int(0.5 * d)
    #B[0, i0, 0] = 1./np.sqrt(2)
    #B[0, i0, 0] = np.random.rand()
    S = np.ones([1], float)
...
def split_truncate_theta(theta, chi_max, eps):
    ...
    "Comment: np.sum(Y > eps) counts the number of elements in Y larger than eps"
    piv = np.argsort(Y)[::-1][:chivC]  # keep the largest `chivC` singular values
    

    
    X, Y, Z = X[:, piv], Y[piv], Z[piv, :]
    norm_new = np.sum(np.square(Y))
    
    eps = 1. - norm_new / norm_old
...
def init_purification_MPS(L, d, bc='finite', state='+x'):
    ...
    if state == '+x':
        ...
        v = np.zeros(ds)
        for n in range(d):
            v[n * d + n] = 1.0 / d     # rho_{nn} = 1/d
        v /= np.linalg.norm(v)         # Frobenius normalise
    else:
        raise ValueError(f"Unknown state '{state}'; use '+x', 'up', or 'mixed'.")
